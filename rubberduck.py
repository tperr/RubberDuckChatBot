...
def classifyQuestion(query, tokenizer, length, categories, model):
    ...
    # classify it
    categoryIndex = np.argmax(model.predict(queryPadded))
    category = list(categories)[categoryIndex]

    # get best answer
    with open("Answers.csv", encoding="iso-8859-1") as file:
...
questionsDataFrame = questionsDataFrame.iloc[:(math.floor(len(questionsDataFrame.values) * 0.3)), [0, 1, 2, 3, 4, 5]]
...
if exists("rubbermodel.h5"):
    ...
    embeddingLayer = Embedding(input_dim=len(tokenizer.word_index) + 1, output_dim=32)(inputLayer)
    convLayer = Conv1D(filters=32, kernel_size=3, activation="relu")(embeddingLayer)
    ...
    model.fit(x=x, y=y, epochs=100, batch_size=32)
...
# test it
while question != "quit":
    print(classifyQuestion(question, tokenizer, length, categories, model))
    question = input("Enter a question: ")
